	...
	var streetview = null;
	var pano_device = 0;
	var fenway = new google.maps.LatLng(-9.525809, -37.292416);
	var openListDevices = false;
	...
	$(document).ready(function () {
		...
		$('#btnAlerts').on('click', function () {
			...
			if ($("#list_info:first").is(":hidden")) {
				$("#list_info").show("slow");
				$("#list_info_title").html("Events");
				openListDevices = false;
				doUpdateListEvents(events);
			} else {
				$("#list_info").slideToggle("slow", function () {
		...
		$("#list_info_search").keyup(function () {
			...
			if (openListDevices == true) {
				...
				var results = _.filter(events, function (item) {
					return item.typeName.toLowerCase().indexOf(search.toLowerCase()) > -1;
				});

				doUpdateListEvents(results);
			}
		});
		...
		$('#btnStreetView').on('click', function () {
			...
			if (currentDevice > 0) {
				...
				doOpenAlertError("Select a vehicle!");
			}
		...
		$('#btnSupport').on('click', function () {
			$(location).attr('href', "http://api.whatsapp.com/send?1=pt_EN&phone=917303326163");
		});
		...
		$.ajax({
			...
			success: function (response) {
				...
				$.each(response, function (key, value) {
					...
					devices[objIndex].protocol = !("protocol" in value) ? "Disconnected" : value.protocol;
					devices[objIndex].time = moment(value.fixTime).format('YYYY-MM-DD HH:mm:ss')
		...
		$.ajax({
			...
			success: function (response) {
				...
				$.each(response, function (key, value) {
					...
					//var nocommunication = getTimeLimit(moment(value.fixTime).format('YYYY-MM-DD HH:mm:ss'));
					var nocommunication = !((device.status=="online" || device.status=="movement")?true:false);
					...
					var html =
						...
						"<div style='font-weight:bold'>" + $.i18n("title_phone") + ":&nbsp;" + (device.phone == null ? '-' : device.phone) + '</div><br />' +
						"<div style='font-weight:bold'>" + $.i18n("title_identifier") + ":&nbsp;" + device.uniqueId + '&nbsp;&nbsp;&nbsp;&nbsp;' +
						"|&nbsp;&nbsp;&nbsp;" + $.i18n("title_model") + ":&nbsp;" + (device.model == null ? '-' : device.model) + '</div><br />' +
						"<div style='font-weight:bold'>" + $.i18n("title_category") + ":&nbsp;" + $.i18n("title_type_" + device.category) + '&nbsp;&nbsp;&nbsp;' +
						"|&nbsp;&nbsp;&nbsp;" + $.i18n("title_device_time") + ":&nbsp;" + moment(value.fixTime).format('DD/MM/YYYY HH:mm:ss') + '</div><br />' +
						"<div style='font-weight:bold'>" + $.i18n("title_coordinates") + ":&nbsp;" + value.latitude.toFixed(6) + ', ' + value.longitude.toFixed(6) + '</div><br />' +
						"<div style='font-weight:bold'>" + $.i18n("title_address") + ":&nbsp;<a href='https://www.google.com/maps/place/" + value.latitude + "," + value.longitude + "/@" + value.latitude + "," + value.longitude + ",17z/data=!3m1!4b1!4m5!3m4!1s0x0:0x0!8m2!3d" + value.latitude + "!4d" + value.longitude + "' target='_blank'>" + value.address + " <img src='assets/img/icons/navigation.png' style='width:16px;height:16px' /></a></div><br />" +
						"<div style='font-weight:bold'>" + $.i18n("title_speed") + ":&nbsp;" + convertSpeed(value.speed, 'knots') + findSpeedUnit(sessionStorage.getItem('speedUnit')).title + '</div><br />' +
						"<div style='font-weight:bold'>" + $.i18n("title_course") + ":&nbsp;" + degToCompass(value.course)+'</div><br />'+
						"<div style='font-weight:bold'>Odometer:&nbsp;"+((value.attributes.odometer== null?"-":value.attributes.odometer))+'</div>';
	...
	function doUpdateListDevices(arr) {
		...
		$.each(arr, function (key, value) {
			...
			var ignitionTitle = (value.ignition == "none" ? "Feature not available" : (value.ignition == true ? "Ignition on" : "Ignition Off"));

			var statusImg = (value.status == "online" ? "status-on" : "status-off");
			var statusTitle = (value.status == "online" ? "Online" : "Offline");

			var movementImg = (value.speed > 5 ? "movement-on" : "movement-off");
			var movementTitle = (value.speed > 5 ? "In motion" : "Stopped");

			//<i class="fa ' + findCategoryType(value.category).icon + '"></i> 
			$("#list_info_body").append('<tr>' +
				'<th scope="row"><span onclick="panTo(\'' + value.deviceId + '\', true)" style="cursor:pointer">&nbsp;' + value.name + '</span></th>' +
				'<td><img src="assets/img/icons/' + movementImg + '.png" style="height: 16px; width: 16px" title="' + movementTitle + '"/></td>' +
				'<td><img src="assets/img/icons/' + statusImg + '.png" style="height: 16px; width: 16px" title="' + statusTitle + '" /></td>' +
	...
	function doUpdateListEvents(arr) {
		...
		$.each(arr, function (key, value) {
			...
			$("#list_info_body").append('<tr>' +
				...
				'<td>' + value.typeName + ' <br/> <span style="font-size:8px; float:right">' + moment(value.serverTime).format('HH:mm:ss') + '</td>' +
...
//limpar icones no mapa
			...
			error: function (response) {
				switch (response.status) {
					case 400:
						...
						doOpenAlertError((/Device is not online/i.test(response.responseText)) ? $.i18n("message_device_not_online") : $.i18n("message_user_not_permission"));
						break;
					case 401:
				...
				$.each(response, function (key, value) {
					...
					switch ($("#report_id").val()) {
						...
						case "0":
							...
							break;
						case "1":
							dataSetReport.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, $.i18n(findEventType(value.type).name), (value.geofenceId > 0 ? findGeofence(value.geofenceId).name : "")]);
							break;
						case "2":
							...
							break;
						case "3":
							dataSetReport.push([findDevice(value.deviceId).name, value.valid, moment(value.fixTime).format('DD/MM/YYYY HH:mm:ss'), value.latitude, value.longitude, convertSpeed(value.speed, 'knots'), degToCompass(value.course), value.address]);
							break;
						case "4":
						...
						case "5":
							...
							if (input1Active == true) {
								var device = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.fixTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.ignition == true ? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots') + ' ' + speedUnit, $.i18n("title_" + device.input1)]);
							}

							if (input2Active == true) {
								var device = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.fixTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.ignition == true ? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots') + ' ' + speedUnit, $.i18n("title_" + device.input2)]);
							}
							break;
						case "6":
				...
				switch ($("#report_id").val()) {
					...
					case "2":
						$("#datatable_report_header").append('<tr>' +
							...
							'<th data-i18n="title_start_address">Start Address</th>' +
							'<th data-i18n="title_end_time">End Time</th>' +
							'<th data-i18n="title_end_address">End Address</th>' +
							'<th data-i18n="title_distance">Distance</th>' +
							'<th data-i18n="title_average_speed">Average Speed</th>' +
							'<th data-i18n="title_maximum_speed">Maximum Speed</th>' +
					...
					case "3":
						$("#datatable_report_header").append('<tr>' +
							'<th data-i18n="title_device_name">Device Name</th>' +
							'<th data-i18n="title_valid">Valid</th>' +
					...
					case "5":
						$("#datatable_report_header").append('<tr>' +
							...
							'<th data-i18n="title_time">Time</th>' +
							'<th data-i18n="title_address">Address</th>' +
							'<th data-i18n="title_ignition">Ignition</th>' +
							'<th data-i18n="title_speed">Speed</th>' +
					...
					case "6":
						$("#datatable_report_header").append('<tr>' +
							...
							'<th data-i18n="title_document">Document</th>' +
							'<th>Email</th>' +
		...
		$.ajax({
			...
			success: function (response) {
				...
				$.each(response, function (key, value) {
					...
					devices[objIndex].ignition = !("ignition" in value.attributes) ? "none" : value.attributes.ignition;
					devices[objIndex].alarm = !("alarm" in value.attributes) ? "" : value.attributes.alarm;
					devices[objIndex].protocol = !("protocol" in value) ? "Unknown" : value.protocol;
					devices[objIndex].time = moment(value.fixTime).format('YYYY-MM-DD HH:mm:ss')
					devices[objIndex].address = (value.address != null ? value.address : "");
					devices[objIndex].positionId = value.id;
	...
	function clearMarkersGroup() {
		for(var i = 0; i < listMarkers.length; i++){
			mymap.removeLayer(listMarkers[i][1]);
		}
	}