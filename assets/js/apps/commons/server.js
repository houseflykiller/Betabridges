    ...
    $('button[name="btnSaveTerm"]').click(function () {
        ...
        $.ajax({
            ...
            success: function (response) {    
                doOpenAlertSucess('Sucess!', 'Data Loaded Successfully', 1500);    
            }
        });
...
function doSaveServer() {
    ...
    doOpenAlertWait('Wait...', 'Writing form!');

    var pars = $("#form_data").serializeObject();		
    	pars.attributes = { name: encrypt(_name),
    						city: encrypt(_city),
...
function doEditServer() {
    ...
    doOpenAlertWait('Wait ...', 'Writting form.');
    ...
    $.ajax({
       type : "GET", 
        ...
        success: function (response) {
            ...
            doOpenAlertSucess('Sucess!', 'Registration Loaded Successfully', 1500);
...
function updatePhoto() {
	...
	$.ajax({
		...
		cache: false,
		error: function (response) {			
            doOpenAlertError('Unable to update logo! \n Contact technical support. \n Error:'+response.statusText);
		},
		success: function (response) {            
            sessionStorage.setItem('logomark', "data:image/png;base64," + _logo);
            $(".logo").html('<img id="site_logomark" src="data:image/png;base64,'+_logo+'" alt="Logo" title="PiGPS Tracker" />');    
		}
