            ...
            $.each(response, function (key, value) {
                ...
                if ($("#status").val() == 'all' || status == $("#status").val()) {        
                    ...
                    if (value.lastUpdate != null) {
                        lastupdate = moment(value.lastUpdate).format('DD/MM/YYYY HH:mm:ss');  

                        var last = moment(value.lastUpdate);
                        var now = moment();                                              
                        countDays = last.diff(now, 'days') ;
                    }                    

                                        
                    dataSet.push([value.id, value.name, $.i18n("title_type_"+value.category), value.model, value.uniqueId, value.phone, value.status, lastupdate, countDays]);
                }
            });
            ...
            $('.datatable-js').dataTable({
		        ...
		        buttons: [{extend: 'excelHtml5',
			               title:  $.i18n("title_reports_status")
                            ...
                            title:  $.i18n("title_reports_status")+"<br />",
                            customize: function ( win ) {
                                var header = '<span style="position:absolute;top:56px;left:0;"><b>'+$.i18n('title_status')+':</b>'+ $("#status").val()+'</span>'+
