...
//	});
    ...
    $('button[name="btnSave"]').click(function () {
        if ($('#form_data').valid()) { doSavePGeoFences(); };
    });
...
function listGeofenceUsers(userId) {
   ...
   $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function (key, value) {
                ...
                html += '    	<li><a href="javascript:doDeletePGeoFences(\'' + value.id + '\',\'' + userId + '\')"><i class="icon-trash"></i> ' +$.i18n("title_delete")+'</a></li>';
                html += '    </ul>';
                html += '</div>';
            ...
            $('.datatable-js').dataTable({
                ...
                buttons: [{
                    extend: 'excelHtml5',
                    title: $.i18n("title_permission_geofences"),
                    exportOptions: {
                        columns: [0, 1]
                    }
                }, {
...
function doSavePGeoFences() {
    ...
    var pars = new Object();        
        ...
        pars.geofenceId = obj.geofenceId;
...
function doDeletePGeoFences(geofenceId, userId) {
