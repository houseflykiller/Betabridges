    ...
    $.ajax({
        ...
        'success': function(data, status, xhr) {
            if (!data.disabled) {
                ...
                sessionStorage.setItem('language', !('language' in data.attributes) ? 'es' : data.attributes.language);
...
function saveUserSettings(settings) {
    ...
    var home_page = 'landing.php';
    switch (sessionStorage.getItem('panel')) {
        case 'communication':
            home_page = 'main.php';
            break;
        default:
            home_page = 'landing.php';
            break;
    }
    $.ajax({
        'type': 'PUT',
...
function configurationServer() {
    ...
    $.ajax({
        ...
        'cache': ![],
        'error': function(response) {
            doOpenAlertError('Error al realizar la operacion, contacte a soport tecnico:' + response.statusText);
        },
        'success': function(data) {
            var data = eval('(' + data + ')');
...
function gpsServer() {
    $.ajax({
        ...
        'error': function(response) {
            switch (response['status']) {
                default: doOpenAlertError($.i18n('message_error_server'));
                break;
            }
        },
        'success': function(data) {
